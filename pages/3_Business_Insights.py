"""
Business Insights page — template-driven BI report with HITL.
...
Template auto-detection picks the best style for the audience (default:
non-technical plain-language story).  The user can override the choice
before generating.
"""
...
# ── HITL Step 2: Template auto-detection with manual override ────────────

auto_template = select_template(
    business_context=business_context,
    analytics_payload=st.session_state.get("analytics_payload"),
)
template_key = "insights_template_override"
if template_key not in st.session_state:
    st.session_state[template_key] = auto_template

with st.expander("Report style", expanded=False):
    st.caption(
        f"Auto-detected: **{describe_template(auto_template)}**. "
        "You can override the style below."
    )
    chosen = st.radio(
        "Choose report style",
        options=["non_technical", "executive", "detailed"],
        format_func=describe_template,
        index=["non_technical", "executive", "detailed"].index(
...
# ── HITL Step 3: Generate / regenerate ─────────────────────────────────
...
if generate_clicked:
    ...
    with st.spinner("Generating business report …"):
        # Override the auto-selected template with the user's choice if they changed it
        chosen_template = st.session_state.get("insights_template_override", auto_template)
        ...
        if report:
            ...
            # Reset follow-up chat on new report
...
if insights_report:
    ...
    st.caption(
        "Drill into any section of the report or ask for more detail. "
        "I'll answer based on the full report and data."
    )
        ...
        with st.chat_message("assistant"):
            with st.spinner(""):
                try:
                    ...
                    # Build system prompt with the full report + analytics payload
                    template_label = st.session_state.get("selected_template_label", "")
                    payload_str = (
            ...
            if fig:
                msg_entry["fig"] = fig
            if result_df is not None:
                msg_entry["df"] = result_df
            insights_messages.append(msg_entry)
